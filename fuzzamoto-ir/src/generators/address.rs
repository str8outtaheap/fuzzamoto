...
/// Build an addr (v1) record with routable IPs and typical service flags.
pub(crate) fn random_addr_v1<R: RngCore>(rng: &mut R, timestamp: u32) -> AddrRecord {
    let (ip_bytes, port) = if rng.gen_bool(0.6) {
        ...
        (ipv4_to_ipv6_mapped(ipv4), random_port(rng, Some(8333)))
    } else {
...
pub(crate) fn random_addr_v2<R: RngCore>(rng: &mut R, timestamp: u32) -> AddrRecord {
    ...
    AddrRecord::V2 {
        ...
        network,
        payload,
        port: random_port(rng, Some(8333)),
    }
}
...
pub(crate) fn random_port<R: RngCore>(rng: &mut R, prefer: Option<u16>) -> u16 {
    ...
    match rng.gen_range(0..100) {
        0 => 18333, // testnet default
        1 => 18444, // regtest default
        2 => 28333, // signet default
...
/// Sample a public, routable IPv4 address.
